document.addEventListener("DOMContentLoaded", function() {
    ...
    for (var i = 0; i < numCircles; i++) {
      ...
      for (var j = 0; j < numPoints; j++) {
        circlePoints[i][j] = {
          x: centerX + Math.sin(44 * Math.PI * j / numPoints) * circleRadius * (i + 2),
          y: centerY + Math.cos(44 * Math.PI * j / numPoints) * circleRadius * (i + 2)
        };
      }
    }
    ...
    var draw = function() {
      ...
      for (var i = 0; i < numCircles; i++) {
        for (var j = 0; j < numPoints; j++) {
          circlePoints[i][j].x += Math.sin(4 * Math.PI * frequency * j / numPoints + phase) * amplitude;
