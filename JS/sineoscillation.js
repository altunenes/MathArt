...
function draw() {
  ...
  for (var i = 0; i < width; i++) {
    ctx.lineTo(i, height / 2+ Math.sin(i * frequency + time) * amplitude);
  }
...
function draw2() {
  ...
  ctx.moveTo(0, height / 2);
  for (var i = 0; i < width; i++) {
    ctx.lineTo(i, height / 2+ Math.cos(i * frequency + time2) * amplitude );
  }
...
function draw3() {
  ...
  for (var i = 15; i < width; i++) {
    ctx.lineTo(i, height / 2+ Math.asin(i * frequency + time) * amplitude );
  }
  ctx.strokeStyle = 'hsl(' + (time * 100) + ', 100%, 50%)';
  ctx.lineWidth = 2;
  ctx.stroke();
